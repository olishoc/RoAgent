import { activateLicense } from "../services/licenseService.ts";
import { getDb, json, readJson, requestMeta, stringField, type HandlerContext } from "./helpers.ts";

export async function handleActivate(request: Request, context: HandlerContext): Promise<Response> {
  ...
  const result = await activateLicense(getDb(context), licenseKey, machineId, requestMeta(request));
  return json(result);
}
