...
function page(title: string, body: string, status = 200): Response {
  return new Response(`<!doctype html><html lang="en"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width, initial-scale=1"/><title>${title}</title><meta name="description" content="RoAgent connects Roblox Studio to an AI coding agent."/><style>${styles}</style></head><body>${body}<footer><span>© 2026 RoAgent</span><nav><a href="/download">Download</a><a href="/recover">Recover</a><a href="/update">Update</a><a href="/privacy">Privacy</a><a href="/terms">Terms</a><a href="mailto:support@rblxagent.com">Support</a></nav></footer></body></html>`, {
    status,
...
function checkoutUrl(env: Env): string {
  ...
  return "mailto:support@rblxagent.com?subject=RoAgent%20access";
}
...
async function handleCheckout(request: Request, env: Env): Promise<Response> {
  ...
  return page("Checkout unavailable — RoAgent", `<main><section class="hero"><p class="eyebrow">Checkout</p><h1>Polar checkout is unavailable.</h1><p class="lede">${escapeHtml(JSON.stringify(data.error || data.detail || response.status))}</p><div class="actions"><a class="button" href="/download">Back to downloads</a></div></section></main>`, 502);
}
...
async function sendRecoveryEmail(env: Env, email: string, links: Record<string, string>): Promise<void> {
  ...
  const from = env.RESEND_FROM || "RoAgent <support@rblxagent.com>";
  const html = `<p>Your RoAgent downloads:</p><ul><li><a href="${links.desktop}">Windows Mission Control app</a></li><li><a href="${links.windows}">Windows daemon</a></li><li><a href="${links.macos}">macOS pkg</a></li><li><a href="${links.plugin}">Roblox plugin</a></li></ul>`;
  await fetch("https://api.resend.com/emails", {
    method: "POST",
    headers: { authorization: `Bearer ${env.RESEND_API_KEY}`, "content-type": "application/json" },
    body: JSON.stringify({ from, to: email, subject: "Your RoAgent downloads", html }),
  });
}
...
async function route(request: Request, env: Env): Promise<Response> {
  ...
  if (request.method === "GET" && url.pathname === "/privacy") return page("Privacy Policy — RoAgent", privacy);
  if (request.method === "GET" && url.pathname === "/terms") return page("Terms — RoAgent", terms);
  if (request.method === "GET" && url.pathname === "/") return home(env);
  return page("Not found — RoAgent", `<main><section class="hero"><p class="eyebrow">404</p><h1>Not found.</h1><p class="lede">That page does not exist.</p></section></main>`, 404);
}
